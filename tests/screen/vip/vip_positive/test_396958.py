...
if platform == "darwin":  # 맥 OS인 경우
    param_json_path = os.path.abspath(os.path.join(current_directory, '..', '..', 'json', 'vip'))
...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1066"]["test_info"]["tr_no"]
            goodsnum = screen["ss_1066"]["input"]["goodsnum"]
            
            try:
                ###Precondition - 상품 URL로 이동
                if goodsnum:
                    item_url = f"https://item.gmarket.co.kr/Item?goodscode={goodsnum}"
                    print(f"# 지마켓 > 이동할 상품 페이지 URL: {item_url}")
                    driver.get(item_url)
                
                else:
                    raise ValueError("# 지마켓 > goodsnum 값이 없습니다.")
                
                # VIP 페이지 로딩 확인
                if not retry_on_failure(ss_1066, vip_page, driver=driver, test_rail=test_rail,
                                        start=start, token=token, output_content=output_content, tr_no=tr_no,
                                        index=index):
                    pytest.fail("ss_1066 failed after retries")
...
# test_396958 실행 함수
def test_396958(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_396958.json')
