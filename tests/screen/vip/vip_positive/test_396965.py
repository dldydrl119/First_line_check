...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1067"]["test_info"]["tr_no"]
            goodsnum = screen["ss_1067"]["input"]["goodsnum"]
...
# test_396965 실행 함수
def test_396965(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_396965.json')
