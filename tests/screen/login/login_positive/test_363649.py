...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1054"]["test_info"]["tr_no"]
            
            try:
                if not retry_on_failure(ss_1054, screen["ss_1054"]["input"], screen["ss_1054"]["assert"], login_page,
...
# test_363649 실행 함수
def test_363649(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_363649.json')
