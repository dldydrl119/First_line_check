...
if platform == "darwin":  # 맥 OS인 경우
    param_json_path = os.path.abspath(os.path.join(current_directory, '..', '..', 'json', 'login'))
...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1055"]["test_info"]["tr_no"]
            
            try:
                if not retry_on_failure(ss_1055, screen["ss_1055"]["input"], screen["ss_1055"]["assert"], login_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no, index=index):
                    pytest.fail(f"ss_1055 failed after retries in test case {tr_no}")
...
# test_387496 실행 함수
def test_387496(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_387496.json')
