...
if platform == "darwin":  # 맥 OS인 경우
    param_json_path = os.path.abspath(os.path.join(current_directory, '..', '..', 'json', 'myg'))
...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1061"]["test_info"]["tr_no"]
            
            try:
                ###Precondition - 로그인 후 마이페이지 이동
                if not retry_on_failure(ss_1054, screen["ss_1061"]["input"], screen["ss_1061"]["assert"], login_page,
                ...
                ########
                # 마이페이지 이동 및 검증
                if not retry_on_failure(ss_1061, myg_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no, index=index):
                    pytest.fail(f"ss_1061 failed after retries in test case {tr_no}")
...
# test_387495 실행 함수
def test_387495(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_387495.json')
