    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            ...
            try:
                ...
                if not retry_on_failure(ss_1054, screen["ss_1063"]["input"], screen["ss_1063"]["assert"], login_page,
                ...
                ########
                # 마이페이지 이동 및 검증
                if not retry_on_failure(ss_1063, myg_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no, index=index):
...
# test_384864 실행 함수
def test_384864(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_384864.json')
