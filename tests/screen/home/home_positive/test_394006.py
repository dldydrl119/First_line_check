...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            ...
            try:
                # Precondition - 로그인 후 홈 페이지 이동
                if not retry_on_failure(ss_1054, screen["ss_1053"]["input"], screen["ss_1053"]["assert"], login_page,
                ...
                # ss_1065 홈 - 로그인 확인
                if not retry_on_failure(ss_1065, screen["ss_1053"]["assert"], home_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no, index=index):
                    pytest.fail(f"ss_1053 failed after retries in test case {tr_no}")
...
# test_394006 실행 함수
def test_394006(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_394006.json')
