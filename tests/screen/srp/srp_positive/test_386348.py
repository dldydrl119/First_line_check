...
if platform == "darwin":  # 맥 OS인 경우
    param_json_path = os.path.abspath(os.path.join(current_directory, '..', '..', 'json', 'srp'))
...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    from src.gmkt_myg_python_dweb_page.gmkt_myg import GmktMygPage
    ...
    try:
        ...
        myg_page = GmktMygPage(driver)
        test_rail = TestRail(config)
        results = []
        
        # 시나리오를 반복 처리
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1063"]["test_info"]["tr_no"]
            
            try:
                ###Precondition - SRP 검색
                # SRP 검색 결과 - 해당 영역 확인
                search_keyword = screen["ss_1063"]["input"]["value1"]  # JSON에서 검색어 가져오기
                search_keyword_assert = screen["ss_1063"]["assert"]["value1"]
                srp_page.go_to_main_page()  # 메인 페이지 이동
                srp_page.enter_search_keyword(search_keyword)  # 검색어 입력
                srp_page.click_search_button()  # 검색 버튼 클릭
                
                srp_page.assert_search_result_url(search_keyword_assert)  # 검색 결과 URL 검증 실행
                ########
                
                # VIP 페이지 로딩 확인
                if not retry_on_failure(ss_1063, screen["ss_1063"], srp_page, driver=driver, test_rail=test_rail,
                                        start=start, token=token, output_content=output_content, tr_no=tr_no,
                                        index=index):
                    pytest.fail("ss_1063 failed after retries")
...
# test_386348 실행 함수
def test_386348(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_386348.json')
