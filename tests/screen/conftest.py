...
if 'mac' in os_version:  # 맥 OS인 경우
    root_path = os.path.dirname(__file__).replace('/tests/screen', '')

else: # 윈도우인 경우
    root_path=os.path.dirname(__file__).replace('\\tests\\screen','')
...
def extract_case_ids_from_files():
    ...
    if 'Windows' in os_version:
        directory_path = root_path + "\\tests\\screen"
    elif 'mac' in os_version:
        directory_path = root_path + "/tests/screen"
...
def cleanup_temp_files(request,config):
    if config["multiple_test_use"]==False:
        ...
        def cleanup_files():
            delete_long_files_in_dir('./tests/screen')
...
def resolve_dynamic_value(json_data, key, dynamic_data=None):
    """
    JSON 데이터에서 값을 추출하고 동적 파라미터를 처리하는 함수
    :param json_data: JSON 데이터
    :param key: 추출할 키 (예: 'input', 'assert')
    :param dynamic_data: 시나리오에서 사용하는 동적 데이터 (optional)
    :return: 처리된 값
    """
    value = json_data[key]
    
    # 동적 파라미터인지 확인
    if isinstance(value, str) and "{%" in value and dynamic_data:
        # 동적 데이터가 있을 경우 치환
        for dynamic_key, dynamic_value in dynamic_data.items():
            value = value.replace(f"{{%{dynamic_key}}}", dynamic_value)
    return value
