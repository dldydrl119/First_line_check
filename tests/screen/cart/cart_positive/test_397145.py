...
if platform == "darwin":  # 맥 OS인 경우
    param_json_path = os.path.abspath(os.path.join(current_directory, '..', '..', 'json', 'cart'))
...
def run_screen_test_case(driver, config, token, screen_json_list):
    ...
    try:
        ...
        for index, screen in enumerate(screen_json_list):
            tr_no = screen["ss_1069"]["test_info"]["tr_no"]
            
            try:
                # Precondition - 로그인 후 카트 페이지 이동
                if not retry_on_failure(ss_1054, screen["ss_1069"]["input"], screen["ss_1069"]["assert"], login_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no):
                    pytest.fail(f"ss_1054 failed after retries in test case {tr_no}")
                cart_page.go_to_cart_page()
                
                # Cart - 장바구니 로드 (상품 있음)
                if not retry_on_failure(ss_1069, cart_page,
                                        driver=driver, test_rail=test_rail, start=start, token=token,
                                        output_content=output_content, tr_no=tr_no):
...
# test_397145 실행 함수
def test_397145(driver, config, token):
    # 시나리오 JSON 파일 불러오기
    screen_json_list = load_json_data(param_json_path + '/test_397145.json')
